...
$success = '';
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      // Basic validation
      if (empty($name) || empty($email) || empty($password) || empty($phone)|| empty($website)) {
      ...
      if (empty($errors)) {
        ...
        try {
      $stmt = $pdo->prepare("INSERT INTO users (name, email, password, phone, website) VALUES (?, ?, ?,?, ?)");
      $stmt->execute([$name, $email, $hashedPassword, $phone, $website]);
  ...
  <div class="row">
    ...
    <div class="col-md-3">
      <div class="form-group">
        ...
        <input type="number" name="phone" class="form-control" required value="<?= htmlspecialchars($_POST['phone'] ?? '') ?>">
      </div>
    </div>
    ...
    <div class="col-md-3">
      <div class="form-group">
        ...
        <input type="text" name="website" class="form-control" required value="<?= htmlspecialchars($_POST['website'] ?? '') ?>">
      </div>
    </div>
    <!-- Submit Button -->
    <div class="col-12 mt-3">
      <button type="submit" class="btn btn-success">Add Customer</button>
      <a href="manageCustomer.php" class="btn btn-secondary ml-2">Cancel</a>
    </div>
  </div>
</form>
