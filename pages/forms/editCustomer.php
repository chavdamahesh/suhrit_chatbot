...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($email) || empty($phone) || empty($website)) {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("UPDATE users SET name = ?, email = ?, phone = ?, website = ? WHERE id = ?");
            $stmt->execute([$name, $email, $phone, $website, $id]);
    ...
    <div class="row">
      ...
      <div class="col-md-3">
        <div class="form-group">
          ...
          <input type="text" name="website" class="form-control" required value="<?= htmlspecialchars($user['website'] ?? '') ?>">
        </div>
      </div>
